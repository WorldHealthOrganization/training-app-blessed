...
export const AccessStep: React.FC<ModuleCreationWizardStepProps> = ({module, onChange}: ModuleCreationWizardStepProps) => {
    ...
    const setModuleSharing = React.useCallback(
        ({userAccesses, userGroupAccesses}: ShareUpdate) => {
                onChange(module => {
                return { ...module, userAccesses: userAccesses ? mapSharingSettings(userAccesses) : module.userAccesses, userGroupAccesses: userGroupAccesses ? mapSharingSettings(userGroupAccesses) : module.userGroupAccesses};
            });
